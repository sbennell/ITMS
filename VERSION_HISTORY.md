...
- Brother DK-22211 (Bordered) label: Assigned To text moved down another 0.5mm (1mm total) - it was still touching the top border.

### Technical Details

- `apps/api/src/services/labelService.ts`: `createLabelPDF()` - the Assigned To text's `y` offset changed from `0.5 * MM_TO_PT` to `MM_TO_PT` (1mm) when `isBordered`.
...
- Brother DK-22211 (Bordered) label: QR code's upward nudge increased another 1.5mm, from 1.5mm to 3mm total (left nudge stays 1.5mm).

### Technical Details

- `apps/api/src/services/labelService.ts`: `createLabelPDF()` - `qrY`'s bordered-variant offset changed from `1.5 * MM_TO_PT` to `3 * MM_TO_PT`.
